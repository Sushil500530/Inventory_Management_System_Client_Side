...
const ProductShopComponent = ({ handleDelete,product }) => {
    ...
    return (
        <div className="card bg-base-100 border dark:bg-zinc-800 dark:text-white shadow-xl pb-6">
            <figure className="w-full h-[250px]">
                <img src={image} alt="image" className="rounded-xl w-full h-full" />
            </figure>
            <div className="flex justify-between items-center mt-5 px-5">
                <h2 className="card-title dark:text-white">{product_name}</h2>
                <h2 className="card-title dark:text-white">$ {product_cost}</h2>
            </div>
            <div className=" px-3 ">
                <div>
                    <p>{description.slice(0, 40)}</p>
                    <p className="text-gray-600 text-[18px] font-bold dark:text-gray-400">Location : <span className="text-black dark:text-white"> {location} </span></p>
                </div>
